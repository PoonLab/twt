...
test_that("sim.arg does not crash at high rho (carrier.id regression)", {
  ...
  # rho=5 previously crashed with "missing value where TRUE/FALSE needed"
  # (rhyper() given a negative population argument once recombination-
  # created lineages inflated count past the host's nominal population
  # size). carrier.id propagation in .do.recombination and .do.coalescent
  # fixes this by counting physical individuals, not tracked lineages.
  arg <- tryCatch(sim.arg(outer, rho=5, seq.length=9000), error=function(e) e)
  expect_false(inherits(arg, "error"))
  expect_true(!is.null(arg$breakpoints))
})

test_that("carrier.id: recombination children inherit parent's carrier.id", {
  parent <- Pathogen$new(name="P1", end.time=1)
  child1 <- Pathogen$new(name="P2", end.time=0.5)
  child2 <- Pathogen$new(name="P3", end.time=0.5)
  child1$set.carrier.id(parent$get.carrier.id())
  child2$set.carrier.id(parent$get.carrier.id())
  expect_equal(child1$get.carrier.id(), parent$get.carrier.id())
  expect_equal(child2$get.carrier.id(), parent$get.carrier.id())
})

test_that("carrier.id: coalescent ancestor inherits a child's carrier.id, not a fresh one", {
  p1 <- Pathogen$new(name="P1", end.time=1)
  p2 <- Pathogen$new(name="P2", end.time=1)
  anc <- Pathogen$new(name="P3", end.time=2)
  anc$set.carrier.id(p1$get.carrier.id())
  expect_equal(anc$get.carrier.id(), p1$get.carrier.id())
  expect_false(anc$get.carrier.id() == anc$get.name())
})
